...
void dfs(int &accum, int node, unordered_set<int> visited, bool visited_small_twice) {
  ...
  if (visited.find(node) != visited.end() 
    ...
    if (type[node] == START || visited_small_twice) {
      return;
    }
  ...
  for (int next : graph[node]) {
    ...
    dfs(accum, next, copy, visited_small_twice);
  }
  return;
}
...
int main() {
  ...
  dfs(result, get_label("start"), visited, false);
